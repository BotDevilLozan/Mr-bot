...
const premium = JSON.parse(fs.readFileSync('./www-fucking/Fenix9991/database/premium.json'))
/**
 * Add premium user.
 * @param {String} userId
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./www-fucking/Fenix9991/database/premium.json", JSON.stringify(_dir));
};
 ...
 * @param {Object} _dir
 */
const expiredCheck = (FenixFucking, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./www-fucking/Fenix9991/database/premium.json", JSON.stringify(_dir));
			idny ? FenixFucking.sendMessage(idny, { text: "Your premium has run out, please buy again." }) : "";
			idny = false;
		}
	}, 1000);
};
